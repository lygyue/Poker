...
#include "Vector3.h"
    ...
    Quaternion Quaternion::Exp () const
    {
        ...
        Radian fAngle ( sqrt(x*x+y*y+z*z) );
        float fSin = sin(fAngle.valueRadians());

        Quaternion kResult;
        kResult.w = cos(fAngle.valueRadians());
	...
	{
        float fCos = Dot(rhs);
        Radian angle = Radian(cos(fCos));

		return (abs(angle.valueRadians()) <= tolerance.valueRadians())
            || RealEqual(angle.valueRadians(), PI, tolerance.valueRadians());
        ...
        const Quaternion& rkQ, bool shortestPath)
    {
        ...
        if (abs(fCos) < 1 - msEpsilon)
        {
            ...
            Radian fAngle = Radian(atan2(fSin, fCos));
            float fInvSin = 1.0f / fSin;
            float fCoeff0 = sin((1.0f - fT) * fAngle.valueRadians()) * fInvSin;
            float fCoeff1 = sin(fT * fAngle.valueRadians()) * fInvSin;
        ...
        const Quaternion& rkP, const Quaternion& rkQ, int iExtraSpins)
    {
        ...
        if ( abs(fAngle.valueRadians()) < msEpsilon )
            return rkP;

        float fSin = sin(fAngle.valueRadians());
        Radian fPhase ( PI*iExtraSpins*fT );
        float fInvSin = 1.0f/fSin;
        float fCoeff0 = sin((1.0f-fT)*fAngle.valueRadians() - fPhase.valueRadians())*fInvSin;
        float fCoeff1 = sin(fT*fAngle.valueRadians() + fPhase.valueRadians())*fInvSin;
